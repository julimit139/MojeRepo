...
class Gracz
{
	...
	int liczbaPunktow = 0;					//pole z liczb¹ punktów gracza
public:
	...
	Karta dobierzKarte1(Karta kartaWylozona);											//funkcja dobieraj¹ca kartê do wy³o¿onej przez cz³owieka - wersja dla pierwszej czêœci gry
	Karta dobierzKarte2(Karta kartaWylozona, int * tab);								//funkcja dobieraj¹ca kartê do wy³o¿onej przez cz³owieka - wersja dla drugiej czêœci gry
	...
	int wybierzKarte(sf::RenderWindow & window, int x, int y, const float X[6], const float Y[2], const float w, const float s);						//funkcja umo¿liwiaj¹ca wybranie karty przez cz³owieka 
